...
<div className={`bg-white/50 backdrop-blur-sm border border-primary rounded-lg shadow p-6 mb-8 text-center ${lang === 'ar' ? 'text-right' : 'text-left'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
  ...
  <p className="text-gray-700">
    {lang === 'ar'
      ? 'تير كلاسيك كوميرشال هو خيار أرضيات داخلية موثوق ودائم ومناسب لمجموعة متنوعة من التركيبات المنزلية والتجارية. مع كل تطبيق، تضمن أرضيات تير كلاسيك التجارية الرضا.'
      : 'TIER Classic commercial is an indoor flooring option that is reliable, durable, and suitable for a variety of home and business installations. With every application, TIER Classic commercial flooring assures satisfaction.'}
  </p>
</div>
      ...
      <div className={`bg-white/50 backdrop-blur-sm border border-primary rounded-lg shadow p-6 mb-8 ${lang === 'ar' ? 'text-right' : 'text-left'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 mb-4">
          <div className="relative w-full max-w-xl mx-auto">
            <img src="/uploads/classic-commercial-diagram-numbers.jpg" alt={lang === 'ar' ? 'مخطط طبقات SPC' : 'SPC Layers Diagram'} className="w-full h-80" />
            {/* Numbered spans, adjust top/left as needed for your image */}
           
          </div>
          <div className="flex-1 space-y-2 text-sm">
            <div><span className="font-bold text-primary">1 {lang === 'ar' ? 'طبقة التآكل' : 'WEAR LAYER'}</span> <span className="text-gray-700">{lang === 'ar' ? 'مقاومة للأشعة فوق البنفسجية والخدش والانزلاق' : 'UV, scratch, and slip resistant'}</span></div>
        ...
        <div className="w-full overflow-x-auto flex flex-col items-center">
          <div className="flex flex-col md:flex-row w-full gap-0 md:gap-4 justify-center">
            ...
            <table className="w-full md:w-2/3 text-xs md:text-sm text-center border-collapse mb-2 md:mb-0 shadow-sm">
              <thead>
                <tr>
                  <th colSpan={3} className="bg-primary text-white py-2 px-2 text-base font-bold">{lang === 'ar' ? 'مواصفات  تير الكلاسيكي' : 'TIER CLASSIC SPECIFICATIONS'}</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr>
                  <td className="py-2 px-2">5.50 مم</td>
                  <td className="py-2 px-2">181 × 1 220 مم</td>
                ...
                <tr>
                  <td className="py-2 px-2">1.00 مم</td>
                  <td className="py-2 px-2">4.00 مم</td>
                  <td className="py-2 px-2">1.95 كجم</td>
                </tr>
                <tr className="bg-gray-100 font-bold">
                  <td className="py-2 px-2">{lang === 'ar' ? 'سماكة طبقة الخارجية' : 'Wear layer thickness'}</td>
                  <td className="py-2 px-2">{lang === 'ar' ? 'مساحة اللوح' : 'm² per board'}</td>
                  <td className="py-2 px-2">{lang === 'ar' ? 'مساحة الصندوق' : 'm² per box'}</td>
                </tr>
                <tr>
                  <td className="py-2 px-2">0.50 مم</td>
                  <td className="py-2 px-2">0.22 م²</td>
                  <td className="py-2 px-2">2.21 م²</td>
                </tr>
                <tr className="bg-gray-200 font-bold">
                  <td className="py-2 px-2">{lang === 'ar' ? 'نوع طبقة الخارجية' : 'Wear layer type'}</td>
                  <td className="py-2 px-2">{lang === 'ar' ? 'نظام التثبيت' : 'Click system'}</td>
                  <td></td>
                </tr>
                <tr>
                  <td className="py-2 px-2">{lang === 'ar' ? 'بوليمير معالج هندسياً' : 'Engineered polymer'}</td>
            ...
            <table className="w-full md:w-1/3 text-xs md:text-sm text-center border-collapse shadow-sm md:border-l md:border-gray-300">
              ...
              <tbody>
                ...
                <tr>
                  <td className="py-2 px-2 border-r border-gray-300">23</td>
