...
export async function createCategory(formData: FormData) {
  try {
    ...
    const slug = generateSlug(name.en)
    let subcategories: { id: string; name: { en: string; ar: string }; slug: string; logo?: string }[] = []
    const subcategoriesRaw = formData.get("subcategories") as string | null
    if (subcategoriesRaw) {
      try {
        const names: { en: string; ar: string; logo?: string }[] = JSON.parse(subcategoriesRaw)
        if (Array.isArray(names)) {
          subcategories = names
            .filter((n) => n && n.en && n.ar)
            .map((n) => ({ id: new ObjectId().toString(), name: { en: n.en, ar: n.ar }, slug: generateSlug(n.en), logo: n.logo }))
        }
      } catch (e) {
    ...
    // Validate category
    const parsed = CategorySchema.safeParse({ name, slug, subcategories });
    if (!parsed.success) {
      throw new Error(parsed.error.errors.map(e => e.message).join(", "));
    }
    const category = {
...
export async function updateCategory(formData: FormData) {
  try {
    ...
    const slug = generateSlug(name.en)
    // Validate update
    const parsed = CategorySchema.partial().safeParse({ name, slug });
    if (!parsed.success) {
      throw new Error(parsed.error.errors.map(e => e.message).join(", "));
    }
    const updateData = {
